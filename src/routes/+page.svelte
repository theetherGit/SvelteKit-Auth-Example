...
{/if}
<div class="my-5 space-y-3 py-12 md:mx-auto md:max-w-3xl lg:pt-16 lg:pb-10">
	<div class="prose max-w-prose mx-auto lg:prose-lg">
		<section id="Introduction">
			...
			<p>
				This is a guide for `SvelteKit 1.0` with Authentication using AuthJS (also known as NextAuth). But it's going to be much more than a guide to SvelteKit.
				Here, I'm going to explain how to add TailwindCSS with DaisyUI, Github OAuth using AuthJS, Theme Switcher from DaisyUI Docs and all SvelteKit basics.
			</p>
		</section>
		...
		<section id="What is SvelteKit">
			...
			<p>
				In simple words we can say, <span>SvelteKit is a framework for building web applications with the Svelte front-end JavaScript library.</span>
				But it comes with much more than anyone can think of and it's all thanks to <ToolTip tip="Creator of Svelte and SvelteKit">`Rich Harris`,</ToolTip>
				Svelte and SvelteKit Team because of them we are seeing SvelteKit v1. Coming back to topic, SvelteKit offers multiple thinks out of the box like:
			</p>
			<ul>
				...
				<li>Universal Loader: Universal load functions that run both on the server and in the browser.</li>
				<li>Adapters: Adapters makes it easy to deploy your app on any place Node Server, Vercel, Cloudflare and etc.</li>
				<li>Data Type Safety: Provides to pre-configured Typescript support and it's way much better if you how to utilize it.</li>
		...
		<section id="Creating new SvelteKit Project">
			...
			<div class="alert alert-info shadow-lg">
				<div>
					...
					<span>
						Before you start, Make sure have installed <span class="bg-white text-gray-900 px-1">Node v16</span> or greater. You can use <span class="bg-white text-gray-900 px-1">npm/yarn/pnpm</span> as package manager it's all upto you.
					</span>
				</div>
			</div>
			<ul>
				...
				<blockquote>When you create SvelteKit project you will get some option choose them according your need.</blockquote>
			</ul>
		</section>
	</div>
	...
	<MockupCode name="Bash">
		...
		>Creating new <a
				...
				target="_blank"
				rel="noreferrer"
				class="bg-white text-gray-900 px-1"><ToolTip tip="MetaFramework for Svelte">SvelteKit</ToolTip></a
		> project</code
		></pre>
		<pre data-prefix="$"><code>pnpm create svelte@latest myApp // To create new sveltekit project</code></pre>
	...
	<MockupCode>
		...
		>Adding <a
				...
				target="_blank"
				rel="noreferrer"
				class="bg-white text-gray-900 px-1"><ToolTip tip="Utility First CSS Framework">TailwindCSS</ToolTip></a
		> and <a href="https://daisyui.com/" target="_blank" rel="noreferrer" class="bg-white text-gray-900 px-1"
		><ToolTip tip="TailwindCSS but sorted">DaisyUI</ToolTip></a
		> to SvelteKit</code
		></pre>
		<pre data-prefix="$"><code
		>pnpm install -D tailwindcss postcss autoprefixer svelte-preprocess</code
		></pre>
	...
	<MockupCode name="svelte.config.js">
		<pre data-prefix="~"><code>Adding Preprocessor for postcss</code></pre><br>
		{#each Object.keys(svelteConfigJS) as line}
			<pre data-prefix={line} class="{svelteConfigJS[line].class}"><code>{svelteConfigJS[line].data}</code></pre>
		{/each}
	</MockupCode>
	<MockupCode name="tailwind.config.cjs">
		{#each Object.keys(tailwindConfigJS) as line}
			<pre data-prefix={line} class="{tailwindConfigJS[line].class}"><code>{tailwindConfigJS[line].data}</code></pre>
		{/each}
	</MockupCode>

	<MockupCode name="{codeFiles[css].filename}">
